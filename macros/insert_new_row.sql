{% macro insert_new_row(source_name, table_name) %}
    ...
    {% set insert_sql %}
        ...
        {%- for col in columns %}
            ...
            {%- if dtype in ('INT64', 'INTEGER', 'INT', 'SMALLINT', 'BIGINT', 'TINYINT', 'BYTEINT') -%}
    ...
    {% do log("Executing INSERT into " ~ src ~ " with random values...", info=True) %}
    {% do run_query(insert_sql) %}
    {% do log("Successfully inserted 1 random row into " ~ src, info=True) %}
